...
def add_footer(slide, current_slide, total_slides=10):
    ...
    tf.margin_left = tf.margin_top = tf.margin_right = tf.margin_bottom = 0
    p = tf.paragraphs[0]
    p.text = "PROTOKOL HUD Manager  •  Комплекс CS2 турнирных трансляций  •  Версия v2.0.0"
    p.font.size = Pt(8.5)
...
def build_presentation(out_path="protokol-presentation.pptx"):
    ...
    badges = [
        ...
        ("ПАМЯТЬ", "30–60 MB RAM", COLOR_GREEN),
        ("ЧАСТОТА ОБНОВЛЕНИЯ", "60 Hz Realtime WS", COLOR_BLUE),
        ("ОБСЛУЖИВАНИЕ", "Zero-Dependency .exe", COLOR_GOLD)
    ]
    bx = 1.1
    ...
    rows_data = [
        ...
        ("Потребление RAM", "300–500 MB (утечки)", "150–250 MB", "30–60 MB (минимально)"),
    ...
    pillars = [
        ("Безотказность на длинных матчах",
         "Нативное ядро Rust исключает утечки памяти и зависания процессов Chromium на тяжелых BO3/BO5 турнирных сериях.",
         COLOR_GREEN),
        ("Автономность без внешнего рантайма",
         "Полноценный независимый бинарник без необходимости устанавливать Node.js, npm-пакеты или веб-серверы на вещательном ПК.",
         COLOR_BLUE),
        ("Честная модульная экосистема",
         "Централизованный интерфейс оператора для мгновенного управления составами, картами, спонсорами и стилями без перезапуска игры.",
         COLOR_GOLD)
    ]
    ...
    s2.notes_slide.notes_text_frame.text = (
        "Слайд 2: Инженерное и финансовое обоснование. LHM и EideticHM создают риски сбоев во время стрима из-за утечек памяти. "
        "PROTOKOL снижает нагрузку на систему вещания на 80-85% и запускается из одного автономного файла."
    )
    ...
    s3.notes_slide.notes_text_frame.text = (
        "Слайд 3: Стиль Fennec Cyber. Особенность: агрессивная, молодежная геометрия. "
        "Скриншот показывает тестовый видеопоток (SMPTE-таймкод) в окне камеры. "
        "В реальном матче при наведении спектатора на игрока здесь мгновенно активируется его веб-камера."
    )
    ...
    items_broadcast = [
        ...
        ("Типографика", "Крупные, мгновенно считываемые цифры здоровья, патронов и статус саппорт-утилит команд."),
    ...
    items_champ = [
        ...
        ("Назначение", "Гранд-финалы, арена-стадии, международные чемпионаты и премиальные шоу-матчи.")
    ]
    ...
    items_webcam = [
        ...
        ("Тестирование из GUI",
         "Оператор может мгновенно проверить статус потока каждого игрока в один клик прямо из таблицы управления камерами.")
    ]
    ...
    add_header(s8, "Инженерия и надежность  •  High Performance",
               "Архитектура сквозного конвейера данных (Data Flow)",
               "Субмиллисекундная обработка пакетов Valve CS2 GSI и векторный радар на 18 соревновательных карт")
    ...
    bullets_1 = [
        ...
        "Прием JSON-пейлоадов от движка CS2 (10–60 раз в секунду).",
        "Совместимый порт-мост :31982 с токеном авторизации.",
        "Парсинг SteamID наблюдаемого игрока с автоматическим фолбэком.",
        "Трекинг фрагов раунда (round_kills) для мгновенной индикации на карточке."
    ]
    for b in bullets_1:
    ...
    bullets_2 = [
        ...
        "Атомарный MatchSnap обновляется в памяти без блокировок потока.",
        "База SQLite в режиме WAL (Write-Ahead Logging) для персистентности сессий.",
        "Полная изоляция процессов: сбой базы не влияет на realtime-трансляцию.",
        "Потребление RAM всего 30–60 MB даже при многочасовом непрерывном вещании."
    ]
    for b in bullets_2:
    ...
    bullets_3 = [
        ...
        "Мгновенная доставка кадров в оверлеи без задержек и артефактов.",
        "Векторный радар CS2 (18 карт, 78 конфигураций уровней).",
        "Удален черный фон: чистые неоновые контуры с мягким glow.",
    ...
    s8.notes_slide.notes_text_frame.text = (
        "Слайд 8: Архитектурная надежность. Разделение ответственности: быстрый сетевой GSI-парсинг на Rust, "
        "трансляция по WebSocket на 60 FPS, изолированная запись в SQLite WAL."
    )
    ...
    v_items = [
        ...
        ("Бэкенд-инженерия", "33 E2E теста бэкенда пройдены успешно; 31 модульный тест Rust и 5 интеграционных тестов SQLite выполнены без ошибок."),
    ...
    l_items = [
        ("Windows Runtime верификация",
         ...
         "Финальное тестирование в рантайме требует физического запуска на Windows с системным WebView2."),
        ...
        ("Отсутствие фиктивных заявлений",
         "Система честно позиционируется как готовый к предтурнирным испытаниям продукт. В релизных материалах исключены нереалистичные бенчмарки и выдуманные статусы «Major certified».")
    ]
    ...
    s9.notes_slide.notes_text_frame.text = (
        ...
        "Инженерная честность повышает доверие руководства: мы четко фиксируем, что доказано автотестами, "
        "а что требует финальной приемочной проверки на Windows-машине режиссера."
    )
    ...
    s_items = [
        ("Снижение риска срыва эфира", "Минимальный след по оперативной памяти (30–60 МБ) и отсутствие тяжелого фонового рантайма гарантируют стабильность на 5+ часовых марафонах."),
    ...
    p.text = "ПЛАН РАЗВИТИЯ СИСТЕМЫ (ROADMAP v2.1+)"
    p.font.size = Pt(11)
    p.font.bold = True
    p.font.color.rgb = COLOR_CYAN
    ...
    s10.notes_slide.notes_text_frame.text = (
        "Слайд 10: Финальное резюме для принятия решения. "
        "Продукт готов к пилотному турнирному запуску. Сформулированы понятные шаги дальнейшего технологического лидерства."
    )
